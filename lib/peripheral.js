...
Peripheral.prototype.syncDescriptor = function(descriptor, callback) {
  // Sort the services to speed things up a bit
  // this.sortServices(function(sortedServices) {
  //   for (var i = 0; i < sortedServices.length; i++) {
  //     var service = sortedServices[i];
  //     if (descriptor.handle >= service._startHandle && descriptor.handle <= service._endHandle) {
  //       var closestChar;
  //       for (var characteristic in service.characteristics) {
  //         if (!closestChar) {
  //           closestChar = service.characteristics[characteristic];
  //         }
  //         else {
  //           if (service.characteristics[characteristic].)
  //         }
  //       }
  //     }
  //   }
...
Peripheral.prototype.clearCache = function(callback) {
  this._allServicesCached = false;
  this._allCharacteristicsCached = false;
  this._allDescriptorsCached = false;
  callback && callback();
}
