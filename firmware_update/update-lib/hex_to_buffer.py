...
def parseCLArguments():
  ...
  parser.add_argument('-output', metavar='output file', type=argparse.FileType('wb'), nargs='?',
                 help='The optional output Node.js file name. Default is ble-firmware.js', default="ble-firmware.image")
...
def generateNodeFile(outputFile, buffer):
  ...
  outputFile.write(array.array('B', buffer).tostring())
  outputFile.close();
  print ("All Finished!")
